...
3) Mount the root and boot drives show with ```lsblk```: Typically for nvme drive its ```mount /dev/nvmen1p2 /mnt``` and ```mount /dev/nvmen1p1 /mnt/boot``` then ```nixos-enter```, exit with ```exit``` before restart use ```umount -a```.
